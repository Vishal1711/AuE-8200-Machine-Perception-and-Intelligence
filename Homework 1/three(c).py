...
x1 = convolve_1d(y,h11k)

plt.plot(n, noise)
plt.plot(n, x1)
plt.plot(n,x)
plt.title('x(k), and y(k) based on kernel window size 11')
plt.legend(['normalized (mean 0, standard deviation 1)','x\'(k)','y(k) based on kernel window size 11','x(k) = 20'])
plt.xlabel('k')
